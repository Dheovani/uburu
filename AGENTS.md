...
- Do not return or declare project-owned aggregate/domain objects with partial designated initializers. Either initialize every field explicitly, or default-construct the object and assign only the semantically relevant fields before returning it. This avoids GCC/Clang `-Wmissing-field-initializers` failures and makes future field additions safer.
