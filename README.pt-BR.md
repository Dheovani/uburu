...
O planejamento completo de evolução e os critérios para a versão 1.0 estão em [TODO.md](TODO.md). Regras de branch, commits e validação local ficam em [docs/development/development.md](docs/development/development.md). O índice completo da documentação está em [docs/README.md](docs/README.md).

Se você quer apenas usar a aplicação, comece pelo [guia do usuário](docs/product/user-guide.pt-BR.md).
...
- SQLite, PCRE2 e libgit2 são detectados pelo core nesta etapa

O manifesto `vcpkg.json` fornece as dependências que não são Qt. O Qt pode ser instalado pelo gerenciador preferido do sistema ou por uma feature de overlay do vcpkg.
...
- vcpkg: obrigatório para dependências que não são Qt; o baseline está fixado em `vcpkg.json`.
...
Use `.env.example` como referência para criar um `.env` local. O arquivo `.env` é ignorado pelo Git. Os scripts PowerShell em `scripts/` carregam `.env` automaticamente; os presets do CMake ainda leem variáveis do ambiente do processo, então defina-as no terminal antes de executar `cmake --preset ...`.
...
O fluxo de empacotamento de release está documentado em [docs/distribution/release.md](docs/distribution/release.md).
...
Confirme a configuração do vcpkg com `Write-Output $env:VCPKG_ROOT`. No PowerShell, use sempre `$env:VCPKG_ROOT`; a forma `$VCPKG_ROOT` pertence a shells POSIX.
...
No Linux, os presets equivalentes são `core-linux-werror-debug` e `core-linux-sanitize-debug`. O workflow em `.github/workflows/ci.yml` executa configure, build, testes, sanitizers e `format-check` para o core.
