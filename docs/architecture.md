...
In the hybrid strategy, the service validates the query once, consults the index, and then runs direct search to confirm the current filesystem view. Reconciliation uses `search::refineSearchResults()`, which classifies results as confirmed, added, or removed and keeps deterministic ordering. Because the current `ResultSink` contract is append-only, hybrid search publishes only the authoritative reconciled direct result set; stale indexed matches are never exposed as final results. Future progressive indexed publication requires explicit correction/removal events rather than pretending an append-only stream can retract stale data.
