...
In the hybrid strategy, the service validates the query once, consults and orders the bounded index result set, and then runs direct search to confirm the current filesystem view. Each authoritative direct result is published progressively and classified as an index hit or miss through ordered lookup, so the service does not retain a second complete direct-result collection. Stale indexed matches are never exposed through the append-only `ResultSink`. `search::refineSearchResults()` remains available for consumers that need explicit confirmed, added, removed, and final sets; future progressive indexed publication requires explicit correction/removal events rather than pretending an append-only stream can retract stale data.
