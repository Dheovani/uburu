...
`SettingsService::exportSettingsAndSavedSearches()` serializes a versioned local export document containing typed global settings and saved searches. The first format is deliberately small and strict: it uses the `uburu-settings-export` signature, version `1`, normalized global settings, and saved searches with name, root, expression, and timestamp. `importSettingsAndSavedSearches()` validates the signature and version before writing anything, then saves global settings and merges saved searches through the storage layer. Existing saved searches with the same name follow the current storage upsert behavior.
