...
Search metrics include derived throughput (`filesPerSecond` and `bytesPerSecond`), queue wait counters, peak occupancy for the worker-task and per-file result queues, index cache hits/misses, reuse by catalog/blob/hash, and an estimate of memory occupied by emitted results. Hybrid search includes its bounded indexed lookup set in that estimate but streams direct validation results without retaining a second complete collection. The memory estimate does not replace a profiler: it sums approximate result structure sizes and observable string/vector capacities. `SearchService` compares that estimate with the previous search to signal growth between executions.
...
The benchmark suite contains many-small-file, dense-match large-file, and sparse-match large-file scenarios for automatic worker selection and fixed counts of 1, 2, 4, and 8 workers. Separating dense and sparse matches distinguishes file-processing throughput from ordered result-publication contention. A 10,000-result stale-index scenario measures ordered hybrid refinement and verifies confirmed, added, removed, and final counts. Further tuning must use reproducible before/after results rather than increasing concurrency blindly.
