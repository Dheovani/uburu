...
Indexed search queries visible documents by worktree root and supports path metadata for `fileName` and `contentAndFileName`. It ignores tombstones (`deleted = true`) and therefore does not return the old path of a locally deleted or renamed file. The first persisted-content support stores normalized text in `documents.indexed_text`, addressed by content hash. This enables real `content` queries without rereading the working tree and without fabricating results from hashes. Document extractors, including DOCX, XLSX, PPTX, OpenDocument, RTF, HTML, subtitles, and bounded PDF extraction, feed this same persisted text path. This representation does not replace a more sophisticated textual backend: tokenization, FTS, compression, ranking, and global highlights remain future evolutions.
...
Milestone 12 introduces `core/document` as the extraction boundary that indexers should use before deciding how to store content. `PlainTextExtractor` is the initial adapter over the existing text reader. `HtmlDocumentExtractor` handles `.html`, `.htm`, and `.xhtml` by indexing visible text, decoding common entities, and excluding scripts, styles, and comments from indexed content. `SubtitleDocumentExtractor` handles `.srt` and `.vtt` by indexing cue text with timestamp locations while ignoring WebVTT headers and note blocks. `RtfDocumentExtractor` handles `.rtf` by extracting visible text, decoding common text escapes, ignoring embedded objects and image destinations, and enforcing group-depth, control-word, and binary-payload limits. `PdfDocumentExtractor` handles the first PDF path for `.pdf` by reading bounded simple unencrypted page content streams, extracting literal and hex text strings, reporting encrypted/protected files explicitly, and publishing page-scoped text segments for direct search, indexing, and preview. `DocxDocumentExtractor` handles the first OOXML path for `.docx` by reading only `word/document.xml` through the shared ZIP layer, extracting paragraph text, tabs, line breaks, and XML entities without exposing raw package XML as searchable text. `XlsxDocumentExtractor` handles the first spreadsheet path for `.xlsx` by reading workbook metadata, shared strings, and worksheet XML through the same ZIP layer, publishing sheet-scoped text segments for direct search, indexing, and preview. `PptxDocumentExtractor` handles the first presentation path for `.pptx` by reading presentation metadata, relationship XML, slide XML, and linked speaker-note XML through the shared ZIP layer, publishing slide-scoped text segments for direct search, indexing, and preview. `OpenDocumentExtractor` handles `.odt`, `.ods`, and `.odp` by reading `content.xml` and `meta.xml` through the shared ZIP layer, publishing document, sheet, or slide-scoped text segments for direct search, indexing, and preview. Rich extractors for legacy Office and message formats must report explicit extraction statuses instead of collapsing all unsupported or unsafe files into generic read failures. The catalog continues to represent the visible file path even when extracted text is unavailable, so file-name search remains correct independently from content extraction. Unsupported formats, binary files, and temporary extraction limitations are published as name-only indexed documents with `indexed_text = NULL` while still incrementing their skip counters.

ZIP-backed document formats such as DOCX, XLSX, PPTX, ODT, ODS, ODP, and EPUB must pass through `core/archive` before format-specific parsing. `ZipArchiveReader` reads the central directory catalog and validates metadata: entry names must be relative and traversal-free, ZIP64 is rejected until explicitly supported, entry count and total expanded bytes are bounded, single-entry expanded bytes are bounded, and suspicious compression ratios are rejected before payload extraction. Payload reading is explicit per entry and supports stored and raw-deflate ZIP entries through zlib with the same safety limits, so document extractors request only the internal XML files they need, such as `word/document.xml`, `xl/sharedStrings.xml`, worksheet XML, slide XML, or manifest files, instead of extracting whole packages. This keeps archive safety reusable across document extractors and prevents the indexer from treating compressed container bytes as searchable text.
