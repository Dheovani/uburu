...
ZIP-backed document formats such as DOCX, XLSX, PPTX, ODT, ODS, ODP, and EPUB must pass through `core/archive` before format-specific parsing. `ZipArchiveReader` currently reads only the central directory catalog and validates metadata: entry names must be relative and traversal-free, ZIP64 is rejected until explicitly supported, entry count and total expanded bytes are bounded, single-entry expanded bytes are bounded, and suspicious compression ratios are rejected before payload extraction exists. This keeps archive safety reusable across document extractors and prevents the indexer from treating compressed container bytes as searchable text.
