...
Milestone 12 introduces `core/document` as the extraction boundary that indexers should use before deciding how to store content. `PlainTextExtractor` is the initial adapter over the existing text reader. Rich extractors for PDF, OOXML, OpenDocument, RTF, HTML, subtitles, and message formats must report explicit extraction statuses instead of collapsing all unsupported or unsafe files into generic read failures. The catalog should continue to represent the visible file path even when extracted text is unavailable, so file-name search can remain correct independently from content extraction.
