...
Direct search processes file contents through a bounded worker pool. The first scanner candidate is processed immediately to preserve progressive time to first result; later files may be read and matched concurrently, while publication remains in scanner order for deterministic output. Each file has a small bounded result queue, the number of in-flight tasks is bounded relative to the worker count, and cooperative cancellation reaches the scanner, task queue, file readers, matchers, and result queues. Automatic mode uses at most eight workers; explicit callers may request a lower or higher count within the supported safety limit.

The benchmark suite contains matching many-small-file scenarios for automatic worker selection and a forced single worker. This makes the throughput and queueing effect of parallel direct search measurable without replacing correctness tests. Further tuning must use reproducible before/after results rather than increasing concurrency blindly.
