...
PDF extraction is intentionally conservative in the Milestone 12 implementation. It supports bounded local parsing of unencrypted PDFs with page-scoped result locations, literal strings, hex strings, UTF-16BE text strings, Windows-1252-compatible single-byte fallback for common accented text, FlateDecode page streams, and simple page-local `/ToUnicode` font maps. It rejects encrypted/protected files as protected content and reports malformed files as parser failures. Uburu does not add a heavyweight PDF backend in the default build for 1.0: Poppler and MuPDF carry licensing/redistribution trade-offs that do not fit the current commercial-reserved license direction, while PDFium remains a possible future optional backend after packaging, binary size, update cadence, and security-maintenance costs are measured. Features such as compressed object streams, advanced custom encodings, forms, annotations, embedded files, JavaScript actions, and layout-aware ordering remain outside the default contract until that backend decision is revisited.
