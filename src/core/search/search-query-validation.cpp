...
namespace uburu::search
{
  namespace
  {
    ...
    {
      if (condition)
        errors.push_back(SearchError{code, std::move(context), std::nullopt});
    }
  ...
  std::vector<SearchError> validate_search_query(const SearchQuery& query)
  {
    ...
    if (!query.root.empty()) {
      ...
      if (error || !exists) {
        errors.push_back(
            SearchError{SearchErrorCode::root_not_found, query.root.string(), std::nullopt});
      } else if (!std::filesystem::is_directory(query.root, error) || error) {
        errors.push_back(
            SearchError{SearchErrorCode::root_not_directory, query.root.string(), std::nullopt});
      }
    }
