...
namespace
{
  ...
  constexpr std::size_t maximumLineLength = 4096;

  std::filesystem::path fuzzRtfPath()
  {
    return std::filesystem::temp_directory_path() / "uburu-rtf-document-extractor-fuzzer.rtf";
  }
...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data, std::size_t size)
{
  ...
  options.textOptions.maximumLineLength = maximumLineLength;

  const uburu::document::RtfDocumentExtractor extractor;
  static_cast<void>(
