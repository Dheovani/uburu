...
{
  uburu::tests::TemporaryDirectory directory("uburu-content-hash-file-test");
  const auto path = directory.path() / "sample.txt";

  uburu::tests::writeFile(path, "abc");
...
{
  uburu::tests::TemporaryDirectory directory("uburu-content-hash-cancel-test");
  const auto path = directory.path() / "sample.txt";
