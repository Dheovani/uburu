...
namespace
{
  ...
  bool hasError(const std::vector<uburu::search::SearchError>& errors, uburu::search::SearchErrorCode code)
  {
    return std::ranges::any_of(errors, [code](const uburu::search::SearchError& error) { return error.code == code; });
  }
